...
import 'package:masterfabric_core_example/features/helpers/force_update/cubit/force_update_demo_state.dart';
...
class ForceUpdateView
  ...
  Widget _buildStatusCard(BuildContext context, ForceUpdateDemoState state,
      ForceUpdateDemoCubit viewModel) {
    return Container(
          ...
          Row(
            children: [
              Icon(
                ...
                size: 20,
                color: state.isInitialized ? AppTheme.success : AppTheme.warning,
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'App Version: ${state.currentVersion ?? 'unknown'}',
                      style: AppTheme.mono
          ...
          Row(
            children: [
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed:
                      state.isInitialized ? viewModel.openStore : null,
  ...
  Widget _buildUITypeSelector(BuildContext context, ForceUpdateDemoState state,
      ForceUpdateDemoCubit viewModel) {
    return Container(
      decoration: AppTheme.cardDecoration,
      child: Column(
  ...
  Widget _buildSimulationCard(BuildContext context, ForceUpdateDemoState state,
      ForceUpdateDemoCubit viewModel) {
    return Container(
      decoration: AppTheme.cardDecoration,
      padding: const EdgeInsets.all(16),
        ...
        children: [
          Row(
            children: [
              ...
              Switch(
                value: state.demoConfig.useSimulation,
                onChanged: (_) => viewModel.toggleSimulationMode(),
              ),
            ],
          ),
          if (state.demoConfig.useSimulation) ...[
            ...
            _buildVersionInput(
              context,
              'Current Version',
              state.demoConfig.simulatedCurrentVersion,
              (value) => viewModel.setSimulatedCurrentVersion(value),
            ),
            const SizedBox(height: 12),
            _buildVersionInput(
              context,
              'Latest Version',
              state.demoConfig.simulatedLatestVersion,
              (value) => viewModel.setSimulatedLatestVersion(value),
            ),
            const SizedBox(height: 12),
            _buildVersionInput(
              context,
              'Minimum Version (optional)',
              state.demoConfig.simulatedMinimumVersion ?? '',
              (value) => viewModel.setSimulatedMinimumVersion(
                  value.isEmpty ? null : value),
            ),
            const SizedBox(height: 16),
            Text(
              'Tip: Set minimum > current for force update',
              style:
  ...
  Widget _buildVersionInput(BuildContext context, String label,
  ...
  Widget _buildPreviewButtons(BuildContext context, ForceUpdateDemoState state,
      ForceUpdateDemoCubit viewModel) {
    return Container(
      decoration: AppTheme.cardDecoration,
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
          ...
          _buildPreviewButton(
            ...
            LucideIcons.info,
            () => _showPreview(context, viewModel, UpdateType.optional),
          ),
          ...
          _buildPreviewButton(
            context,
            'Recommended Update',
            'Show recommended update prompt',
            LucideIcons.triangleAlert,
            () => _showPreview(context, viewModel, UpdateType.recommended),
          ),
          const Divider(height: 16),
          ...
          _buildPreviewButton(
            ...
            () => _showPreview(context, viewModel, UpdateType.force),
          ),
        ],
      ),
    );
  ...
  Widget _buildPreviewButton(BuildContext context, String title,
  ...
  void _showPreview(BuildContext context, ForceUpdateDemoCubit viewModel,
      UpdateType updateType) {
    final state = viewModel.state;
    
    // Create update info based on simulation or real data
    UpdateInfo info;
    if (state.demoConfig.useSimulation) {
    ...
    // Show based on selected UI type
  ...
  Widget _buildVersionComparisonCard(BuildContext context) {
    final comparisons = [
      ...
      ('2.0.0', '1.0.0', 'newer'),
      ('1.0.0', '1.0.0', 'equal'),
      ('1.0.0-beta', '1.0.0', 'older (pre-release)'),
      ('1.0.0+1', '1.0.0+2', 'older (build)'),
    ];
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _getComparisonColor(result).withValues(alpha: 0.1),
                    ...
                    child: Text(
                      ...
                      style: AppTheme.mono.copyWith(
                        fontSize: 10,
                        color: _getComparisonColor(result),
                      ),
                    ),
                  ),
                ],
  ...
  String _getUITypeDescription(UpdateUIType type) {
    switch (type) {
      case UpdateUIType.dialog:
        return 'Compact dialog overlay';
      case UpdateUIType.bottomSheet:
        return 'Modal from bottom';
      case UpdateUIType.fullScreen:
        return 'Blocking full page';
    }
  }
}
