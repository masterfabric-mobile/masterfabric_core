...
import 'package:masterfabric_core_example/features/helpers/push_notification/cubit/push_notification_state.dart';
...
class PushNotificationView
      ...
      builder: (context, state) {
        ...
        return ListView(
          ...
          children: [
            ...
            _buildTopicsCard(context, state, viewModel),
            const SizedBox(height: 24),

            // User ID
            _buildSectionTitle(context, 'User Targeting'),
            _buildUserIdCard(context, viewModel),
            const SizedBox(height: 24),

            // Recent Notifications
            _buildSectionTitle(context, 'Recent Notifications'),
            _buildNotificationsCard(context, state),
            const SizedBox(height: 24),
      ...
      child: Column(
        children: [
          Row(
            children: [
              Icon(
                state.isInitialized ? LucideIcons.circleCheck : LucideIcons.circleAlert,
                size: 20,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      state.isInitialized
                          ? '${state.activeProviders.length} provider(s) active'
                          : 'Tap to initialize',
          ...
          if (state.config != null) ...[
            ...
            _buildConfigRow('Default Provider', state.config!.defaultProvider.name),
            _buildConfigRow('Auto Initialize', state.config!.autoInitialize.toString()),
          ],
        ],
      ),
    );
  ...
  Widget _buildProvidersCard(BuildContext context, PushNotificationDemoState state) {
    if (!state.isInitialized) {
      return _buildEmptyCard('Initialize to see active providers');
    }

    if (state.activeProviders.isEmpty) {
      return _buildEmptyCard('No providers configured');
    }
  ...
  Widget _buildPermissionsCard(
    ...
    if (!state.isInitialized) {
      return _buildEmptyCard('Initialize to check permissions');
    }
          ...
          ...state.permissionStatuses.entries.map((entry) {
            ...
            return Padding(
              ...
              child: Row(
                children: [
                  Icon(
                    isGranted ? LucideIcons.bell : LucideIcons.bellOff,
                    size: 16,
  ...
  Widget _buildTokensCard(BuildContext context, PushNotificationDemoState state) {
    if (!state.isInitialized) {
      return _buildEmptyCard('Initialize to see device tokens');
    }
            ...
            child: Row(
              children: [
                Icon(
                  LucideIcons.key,
                  size: 16,
                  color: token != null ? AppTheme.success : AppTheme.textMuted,
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
  ...
  Widget _buildNotificationsCard(BuildContext context, PushNotificationDemoState state) {
    if (state.recentNotifications.isEmpty) {
      return _buildEmptyCard('No notifications received yet');
    }

    return Container(
      decoration: AppTheme.cardDecoration,
      child: Column(
        children: state.recentNotifications.asMap().entries.map((entry) {
          final index = entry.key;
          ...
          return Column(
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      LucideIcons.bell,
                      size: 16,
                      color: _getProviderColor(notification.source),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${notification.source.name} • ${_formatTime(notification.receivedAt)}',
                            style: AppTheme.mono.copyWith(fontSize: 10, color: AppTheme.textMuted),
                          ),
                        ],
                      ),
              ...
              if (index < state.recentNotifications.length - 1) const Divider(),
            ],
          );
        }).toList(),
      ),
  ...
  Color _getProviderColor(PushNotificationProviderType type) {
    switch (type) {
      case PushNotificationProviderType.onesignal:
        return const Color(0xFFE54B4D);
      case PushNotificationProviderType.firebase:
